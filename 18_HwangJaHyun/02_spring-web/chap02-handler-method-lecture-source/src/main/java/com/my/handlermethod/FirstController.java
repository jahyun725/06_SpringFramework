...
@Controller //bean등록 컨트롤러역할
@RequestMapping("/first") // first로 시작하는 모든 것
@SessionAttributes({"pwd"})
public class FirstController {
  // 반환형 String / void / ResponseEntity


  /* 핸들러 메서드의 반환형이 void이면
  *  요청 주소가 곧 view의 이름이 된다.
  *  요청 주소 : /first/regist
  *  view(html) : /resources/templates/first/regist
  * */
  @GetMapping("/regist")
  public void regist(){}

  /* ArgumentResolver(전달인자 해결사)
   * - 핸들러 메서드(컨트롤러 메서드)가 호출될 때
   *   매개 변수에 작성된 변수에 맞게 인자를 준비해주는 해결사
   *
   * - ArgumentResolver 리스트에 작성된 타입이 호출되면
   *   자동으로 생성 또는 찾아서(이미 존재할 경우) 주입
   * */

   /* 1. WebRequest로 요청 파라미터 전달 받기
   *  - [Servlet] HttpServletRequest를 이용해서 파라미터를 얻어올 수 있음
   *  - WebRequest를 이용해 Servlet에 종속되지 않는 Spring 기반 객체 사용.
   * */
  @PostMapping("/regist")
  public String registMenu(Model model, WebRequest req){
    ...
    String message = String.format("%s을/를 신규 메뉴 목록의 %d번 카테고리에 %d원으로 등록했습니다.", name, categoryCode, price);
    model.addAttribute("message", message);

    return "first/messagePrinter";
  }

  // void : /first/modify.html로 forward해라
  @GetMapping("/modify")
  public void modify(){}
  ...
  /* 3. @ModelAttribute
  * 제출되는 파라미터의 name이 @ModelAttribute 옆에 작성된 객체의 필드명과 같다면
  * 해당 객체의 setter를 이용해서 파라미터를 필드에 세팅한다.
  * -> 전달 받은 파라미터를 저장한 객체 == 커맨드 객체
  * (@ModelAttribute MenuDTO menuDTO) << menuDTO가 커맨드 객체
  * @ModelAttribute("menu") << 이름(속성명)을 지정 해주면
  ...
  * */
  ...
  /* 4. @SessionAttributes
  *  - Spring에서 제공하는 Session을 다루는 annotation
  *  - 클래스 레벨에 작성
  *  - 단독 사용 X, Model 객체와 같이 사용
  *  - 사용법
  *    1) @SessionAttributes({"key1", "key2"}}) 작성
  *    2) Model.addAttribute("key1", "value1");
  *      -> @SessionAttributes에 등록된 "key1"이라는 key를 이용해서
  *         Model의 속성으로 추가하게 되면
  *         request스코프가 아닌 Session scope로 추가된다.
  *
  * */
  //model 화면과 데이터를 교환하는 객체
  @PostMapping("login")
  public String loginTest(@RequestParam("id") String id, @RequestParam("pwd") String pwd, Model model){

    // Model에 속성을 추가하는 경우, scope는 기본적으로 request scope
  ...
  /* @SessionAttributes 만료
  *  - SessionStatus라는 세션 상태를 관리하는 스프링 객체를 얻어와
  *    setComplete()로 세션을 만료시킨다.
  *    button onclick << GET방식
  *  - HttpSession은 Servlet 기반 객체로
  *    @SessionAttributes와는 따로 관리가 되고 있다!
  *    -> 둘을 혼용해서 사용하고 있다면 만료시 SessionStatus.setComplete()
  ...
  * */
  ...
  public String logoutTest(SessionStatus status) {
    ...
    status.setComplete(); // 세션 만료 전 -> id만 사라짐
  ...
  // GET 불가능, 애초에 쿼리스트링형태로 헤더로 전달되기 때문에
  @PostMapping("/body")
