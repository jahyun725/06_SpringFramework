...
public class FileUploadController {
  ...
  @Value("${spring.servlet.multipart.location}") // yaml에 써있는 경로
  ...
  * */
  ...
  public String singleFileUpload(
    ...
    System.out.println("제출 파일명 = " + singleFile.getOriginalFilename());
    System.out.println("파일크기(Byte) = " + singleFile.getSize());
    ...
    // 2. 서버에 저장할 파일명 변경하기
    // * 같은 폴더 내에는 같은 이름의 파일이 중복 저장될 수 없다.(덮어쓰기 됨..)
    String savedName = generateSavedFileName(singleFile);

    // 3. 지정된 경로에 변경된 이름으로 파일 저장하기
    // * 파일 저장 전 파일은 어디에 있을까?
    // -기본적으로는 메모리에 로드 되어있다
    // - 설정해둔 메모리 제한 용량보다 파일이 크면 지정된 임시 경로에 저장
    try {
  ...
  // 네트워크 상에서 중복되지 않는 유일한 아이디를 만들 때 사용하는 표준 규약
  // 128비트 숫자로 이루어짐(출력시 16진수로 변환돼서 보임)
  ...
  public String multiFileUploadDB(
    @RequestParam String multiFileDescription,
    @RequestParam List<MultipartFile> multiFile,
    Model model
  ){
    ...
    try{
      ...
      for(FileDTO f : files){
        File file = new File(f.getFilePath() + "/" + f.getSavedName());
